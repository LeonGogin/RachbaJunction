...
class WaveFunction():
    ...
    def omega_k(self, x, k, b):  
        ...
        res = norm*np.array([np.exp(complex(0,-self.phi))*(k[0] - b*np.sqrt(k[0]**2 + 1)), 1], dtype=np.complex256)

        # norm = np.sqrt(1/(2*np.sqrt(1 + k[0]**2)))
        # res = norm*np.array([-b*np.exp(complex(0,-self.phi))*np.sqrt( -b*k[0] + np.sqrt(k[0]**2 + 1)), 1/np.sqrt( -b*k[0] + np.sqrt(k[0]**2 + 1))], dtype=np.complex256)
    ...
    def omega_q(self, x, q, b):
        norm = np.sqrt(1/2)
        res = norm*np.array([np.exp(complex(0,-self.phi))*(-complex(0,q[0]) - b*np.sqrt(1 - q[0]**2)), 1], dtype=np.complex256)
    ...
    def calculate_velocity(self, E):
        ...
        for i in range(len(self.wave_length)):
            ...
            # evanescent velocities
            # elif self.mod[i] == "q" and np.sign(self.wave_length[i][1]) < 0 and k_s < 0:
            #     vel = -np.sign(self.wave_length[i][1])*(np.sqrt(self.E_0(E, self.l[i], self.mod[i])*(1 - q**2)) + self.band[i]*np.abs(q)*np.sqrt(self.E_so))/np.sqrt(1 - q**2)
            #     self.r_q_vel = vel
            # elif self.mod[i] == "q" and np.sign(self.wave_length[i][1]) > 0 and k_s > 0:
            #     vel = -np.sign(self.wave_length[i][1])*(np.sqrt(self.E_0(E, self.l[i], self.mod[i])*(1 - q**2)) + self.band[i]*np.abs(q)*np.sqrt(self.E_so))/np.sqrt(1 - q**2)
            #     self.l_q_vel = vel
...
class RachbaJunction(WaveFunction):
    _m = 0.022*cc.m_e
    def __init__(self, profile = [[],[]], logg = False):
        ...
        super(RachbaJunction, self).__init__()
    ...
    def get_transfer_matrix(self, E):
        ...
        if len(self.vel_a) == 2:
            ...
            q_v = np.lib.scimath.sqrt(np.array([self.l_q_vel, self.r_q_vel]).reshape(2, 1))
        ...
        # is wrong!!!!
        if vel_factor_mat.shape[0] == 2:
            self.evanescent_vell_factor = np.dot(q_v, v_a)
